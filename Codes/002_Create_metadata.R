...
####### 2. Load data ######
###########################
reads_bowtie <- read.table(file.path(main_wd,input_dir,"reads_bowtie.txt"),check.names = F)
reads_new    <- read.table(file.path(main_wd,input_dir,"reads_new.txt"),check.names = F)
...
reads_bowtie <- reads_bowtie[,sample_name]
reads_new <- reads_new[,sample_name]

reads_nc <- reads_bowtie[which(rownames(reads_bowtie)!=rownames(reads_new[rownames(reads_bowtie),])),]
...

{
  ...
  dir.create("Inputs/002_Processed_data/xlsx",showWarnings = F,recursive = T)
  dir.create("Inputs/002_Processed_data/txt",showWarnings = F,recursive = T)
  dir.create("Inputs/002_Processed_data/RDS",showWarnings = F,recursive = T)

  ### Save data
  write_xlsx(metadata,file.path(getwd(),"Inputs/002_Processed_data/xlsx/metadata_66_samples.xlsx"))
  write.table(metadata,file.path(getwd(),"Inputs/002_Processed_data/txt/metadata_66_samples.txt"))
  write.table(reads,file.path(getwd(),"Inputs/002_Processed_data/txt/reads_66_samples.txt"))
