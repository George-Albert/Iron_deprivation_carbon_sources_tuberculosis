...
file_name <- c("enhanced_upregulated.txt","Dampened_upreg.txt","enhanced_downregulated.txt","Dampened_downreg.txt")
...
for (i in vec) {
  ...
  ### Dampened_DOWN:
  summary$label_int[which( summary$logFC_with<0 & summary$logFC_without<0 &
                             summary$BH_int<threshold_int & summary$logFC_int>0)]="Dampened_DOWN"
  Dampened_down_genes <- summary[which(summary$label_int %in% c("Dampened_DOWN")),]
  print(paste("there are", length(Dampened_down_genes$label_int), "Dampened in the down direction"))

  down_chunk=summary[which(summary$label_int %in% c("Enhanced_DOWN","Dampened_DOWN")),]
  ...
  ### Dampened_UP:
  summary$label_int[which( summary$logFC_with>0 & summary$logFC_without>0 &
                             summary$BH_int<threshold_int & summary$logFC_int<0)]="Dampened_UP"
  Dampened_up_genes <- summary[which(summary$label_int %in% c("Dampened_UP")),]
  print(paste("there are", length(Dampened_up_genes$label_int), "Dampened in the up direction"))


  ### Short classification
  summary$class4 <- "Non_Coherent"

  summary$class4[summary$label_int=="Enhanced_UP"]   <- "EU"
  summary$class4[summary$label_int=="Enhanced_DOWN"] <- "ED"
  summary$class4[summary$label_int=="Dampened_DOWN"] <- "DD"
  summary$class4[summary$label_int=="Dampened_UP"]   <- "DU"

  ### Save dataframe
  carbon_results[[name[indice]]] <- summary
  ...
  up_chunk=summary[which(summary$label_int %in% c("Enhanced_UP","Dampened_UP")),]
  ...
  fraction[4] <- 100*nrow(summary[which(summary$label_int %in% c("Dampened_DOWN")),])/sum(summary_df[3:7,])
  fraction[5] <- 100*nrow(summary[which(summary$label_int %in% c("Dampened_UP")),])/sum(summary_df[3:7,])
  ...
  print(paste("the fraction of Dampened_DOWN is",fraction[4]))
  print(paste("the fraction of Dampened_UP is",fraction[5]))
  print(paste("the total fraction of coherent responses is",fraction[6]))

  fraction_df <- data.frame(Description=c("non_coh_vs_coh","Enhanced_DOWN","Enhanced_UP","Dampened_DOWN",
                                          "Dampened_UP","total fraction of coherent responses"),Fraction=fraction)
...
carbon_results_final <- list(
  "G-D"    = carbon_results[["C1_C2_Glycerol_Dextrose"]],
  "G"      = carbon_results[["C5_C6_Glycerol"]],
  "G-LCFA" = carbon_results[["C7_C8_Glycerol_LCFA"]]
)

filtered_carbon_results <- lapply(carbon_results_final, function(df) {

  df_filtered <- df[df$class4 %in% c("EU","ED","DD","DU"), ]

  df_filtered$class4 <- NULL
...
### Locate dampened and enhanced genes in the summary table

dd_gd <- rownames(filtered_carbon_results[["G-D"]][filtered_carbon_results[["G-D"]]$label_int == "Dampened_DOWN", ])
dd_g  <- rownames(filtered_carbon_results[["G"]][filtered_carbon_results[["G"]]$label_int == "Dampened_DOWN", ])
dd_gl <- rownames(filtered_carbon_results[["G-LCFA"]][filtered_carbon_results[["G-LCFA"]]$label_int == "Dampened_DOWN", ])

common_DD <- Reduce(intersect, list(dd_gd, dd_g, dd_gl))
...
du_gd <- rownames(filtered_carbon_results[["G-D"]][filtered_carbon_results[["G-D"]]$label_int == "Dampened_UP", ])
du_g  <- rownames(filtered_carbon_results[["G"]][filtered_carbon_results[["G"]]$label_int == "Dampened_UP", ])
du_gl <- rownames(filtered_carbon_results[["G-LCFA"]][filtered_carbon_results[["G-LCFA"]]$label_int == "Dampened_UP", ])

common_DU <- Reduce(intersect, list(du_gd, du_g, du_gl))
length(common_DU)
...
# fraction_DU <- 100*final_summary["Dampened_UP",1]/sum_total_genes
# fraction_DD <- 100*final_summary["Dampened_DOWN",1]/sum_total_genes
...
categorie <- factor(rownames(final_summary[3:7,]),levels = c("Non_Coherent","Dampened_DOWN","Dampened_UP",
                                                             "Enhanced_DOWN","Enhanced_UP"))
...
interactions <- factor(
  interactions,
  levels = c("LCFA", "Glycerol_LCFA", "Glycerol", "Glycerol_Dextrose"),
  labels = c("LCFA", "G-LCFA", "G", "G-D")
)
...
bar_plt <- ggplot(df, aes(x = interactions, y = values, fill = categorie)) +
  geom_bar(
    position = "stack",
    stat = "identity",
    width = 0.7,
  ...
  labs(x = "Carbon source", y = "Number of genes") +
  theme_classic() +
...
bar_pl_1 <- bar_plt +
  geom_shadowtext(
    data = subset(df, interactions != "LCFA"),
    aes(label = values),
...
pdf(file = file.path(output_dir,"001_Figures_paper","12_Figure_3C_Stacked_Bar_plot_interactions_enhanced.pdf"),width=8,height=5)
print(bar_pl_1)
dev.off()
