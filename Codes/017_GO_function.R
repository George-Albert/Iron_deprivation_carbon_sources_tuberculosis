...

{
  ...
  output_dir <- "Outputs"
  load(file.path(input_folder,"boolean_matrix_bp"))
  load(file.path(input_folder,"BP_metadata_matrix"))
  load(file.path(input_folder,"boolean_matrix_cc"))
  load(file.path(input_folder,"CC_metadata_matrix"))
  ...
  ###This matrix contains information about terms related to cellular component and their genes
...
file_name <- all_file_name[ !grepl("trash", all_file_name) ]
list_DE_genes <- lapply(file.path(dir_of_gene_list,file_name), read.table)

names(list_DE_genes) <- file_name
condition_name <- list.files(dir_of_gene_list)
condition_name <- condition_name[c(1:4)]

list_downreg_genes <- list_DE_genes[c(1,3,5,7,9,11,13,15)]
list_enhanced_downreg_genes <- list_downreg_genes[c(2,4,6,8)]
names(list_enhanced_downreg_genes) <- condition_name
list_damped_downreg_genes <- list_downreg_genes[c(1,3,5,7)]
names(list_damped_downreg_genes) <- condition_name

list_upreg_genes <- list_DE_genes[c(2,4,6,8,10,12,14,16)]
list_enhanced_upreg_genes <- list_upreg_genes[c(2,4,6,8)]
...
vector_of_names <- names(list_enhanced_downreg_genes)

# list_of_foldername_to_use <- c("Enhanced_UP","Enhanced_Down","Enhanced_UP_2nd_Config","Enhanced_Down_2nd_Config",
#                                "Enhanced_UP_twosided","Enhanced_DOWN_twosided" )
...
list_var <- list("list_enhanced_upreg_genes"=list_enhanced_upreg_genes,"list_enhanced_downreg_genes"=list_enhanced_downreg_genes)
enhanced_vec <- c("Enhanced_UP_twosided","Enhanced_DOWN_twosided")

for (j in seq_along(list_var)) {

  print(names(list_var[j]))
  list_to_loop <- list_var[[j]]
  type_of_enhanced <- enhanced_vec[j]
  print(type_of_enhanced)

  for (i in seq_along(list_to_loop)) {
    ...
    query_genes <- unlist(list_to_loop[i])
    ...
    dir.create(enrichment_dir,recursive = T,showWarnings = F)
