...
dir_of_gene_list <- "Outputs/Enhanced_and_damped_genes"
...
new_names <- names(lista_ED)

new_names <- gsub("C1_C2_Glycerol-Dextrose", "G-D", new_names)
new_names <- gsub("C5_C6_Glycerol", "G", new_names)
new_names <- gsub("C7_C8_Glycerol-LCFA", "G-LCFA", new_names)

new_names <- gsub("/Enhanced_DOWN_twosided/Enrichment_GO.txt", "_ED", new_names)
names(lista_ED) <- new_names

new_names <- names(lista_EU)
new_names <- gsub("C1_C2_Glycerol-Dextrose", "G-D", new_names)
new_names <- gsub("C5_C6_Glycerol", "G", new_names)
new_names <- gsub("C7_C8_Glycerol-LCFA", "G-LCFA", new_names)

new_names <- gsub("/Enhanced_UP_twosided/Enrichment_GO.txt", "_EU", new_names)
...
# Raw OR matrix
hclust_matrix_raw <- as.matrix(df[, c(2, 6, 10)])
colnames(hclust_matrix_raw) <- c("G_D", "G", "G_LCFA")

# Log2(OR)
plot_matrix <- log2(hclust_matrix_raw)
...
min_finite_up <- min(cluster_matrix[is.finite(cluster_matrix)], na.rm = TRUE)
cluster_matrix[is.na(cluster_matrix)] <- min_finite_up - 0.5

# Clustering
dist_matrix <- dist(cluster_matrix)
OR_clust <- hclust(dist_matrix, method = "ward.D2")
col_dend <- hclust(dist(t(cluster_matrix)))

columns_name <- colnames(plot_matrix)
column_title <- "Conditions"
clust_name <- "GO"

# Color scale
break_vec <- c(0, (max_cap/2)-2, max_cap-2)
color_breaks <- break_vec
col_fun <- colorRamp2(color_breaks, c("white", "#fcae91", "#cb181d"))
...
ht_plt <- Heatmap(
  ...
  cell_fun = function(j, i, x, y, width, height, fill) {
    if (!is.na(plot_matrix[i, j])) {
      grid.text(round(plot_matrix[i, j], 2), x, y, gp = gpar(fontsize = 3))
    }
  },
...
cluster <- dendextend:::cutree(OR_clust, k = k, order_clusters_as_data = FALSE)
idx <- match(rownames(cluster_matrix), names(cluster))
hcl_matrix <- cbind(cluster_matrix, cluster = cluster[idx])
...
write.table(
  hclust_df_up,
  file = file.path(input_folder, paste0("Cluster_OR_", OR, "_fdr_", fdr, "_up_k=", k, ".txt"))
)

dir.create(file.path(output_dir, "001_Figures_paper"), recursive = TRUE, showWarnings = FALSE)
filename     <- file.path(output_dir, "001_Figures_paper", "13_Figure_4A_Heatmap_GO_up.pdf")
svg_filename <- file.path(output_dir, "001_Figures_paper", "13_Figure_4A_Heatmap_GO_up.svg")
...
# Raw OR matrix
hclust_matrix_raw <- as.matrix(df_down[, c(2, 6, 10)])
colnames(hclust_matrix_raw) <- c("G_D", "G", "G_LCFA")
...
# Matrix for clustering
cluster_matrix <- plot_matrix
min_finite_down <- min(cluster_matrix[is.finite(cluster_matrix)], na.rm = TRUE)
cluster_matrix[is.na(cluster_matrix)] <- min_finite_down - 0.5
...
break_vec <- c(0, (max_cap/2) - 2, max_cap - 2)
color_breaks <- break_vec
col_fun <- colorRamp2(color_breaks, c("white", "#9ecae1", "#08519c"))
...
ht_plt1 <- Heatmap(
  ...
  border_gp = gpar(col = "black", lty = 2),
  width = unit(2, "cm"),
  cluster_rows = color_branches(OR_clust, k = k, groupLabels = TRUE),
  # cluster_columns = color_branches(col_dend),
  row_title = clust_name,
  row_title_gp = gpar(fontsize = 6),
  row_dend_side = "right",
...
write.table(
  hclust_df_down,
  file = file.path(input_folder, paste0("Cluster_OR_", OR, "_fdr_", fdr, "_down_k=", k, ".txt"))
)

filename     <- file.path(output_dir, "001_Figures_paper", "13_Figure_4A_Heatmap_GO_down.pdf")
svg_filename <- file.path(output_dir, "001_Figures_paper", "13_Figure_4A_Heatmap_GO_down.svg")
